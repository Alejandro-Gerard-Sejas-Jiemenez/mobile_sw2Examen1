...
import { BYTES_PER_MB, TARGET_MODEL_SPECS } from '@/constants/ai.constants';
...
export function useAiModelManager() {
  ...
  const handleDownload = useCallback(async () => {
    try {
      ...
      Alert.alert(
        '¡Modelo Listo!',
        `${TARGET_MODEL_SPECS.NAME} cargado correctamente en tu dispositivo.`
      );
    } catch (error) {
      setDownloading(false);
      console.error('[useAiModelManager] Download error:', error);
      Alert.alert(
        'Error de Descarga',
        'No se pudo descargar el modelo. Revisa tu conexión a internet.'
      );
    }
  }, []);
  ...
  const handleDelete = useCallback(async () => {
    Alert.alert(
      'Eliminar Modelo',
      `¿Deseas eliminar el modelo local de IA para liberar ${TARGET_MODEL_SPECS.SIZE_FORMATTED}?`,
      [
